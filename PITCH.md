...
| 4 | *"Best first-line antibiotic for community-acquired pneumonia in adults 2024"* | **Live-multi-AI showstopper.** Out of corpus → `NotFoundScreen` → click *Search live* → 3-stage progress (PubMed → Gemini → Llama) → cited answer + "Added N articles to KB" chip | *"Watch what happens when our cache doesn't have it. PubMed → Gemini synthesizes → Llama verifies. ~20 seconds, fully cited, and now in the cache for next time."* |
...
**CRITICAL:** Do **NOT** click query #4 before stage time. The moment you do, write-back absorbs the articles into the cache and the showstopper is gone. Backups if it gets contaminated: *"Recommended dose of levetiracetam for status epilepticus"* or *"First-line eradication therapy for H. pylori 2024"*.
...
| 1:10–1:35 | **Hero query #4** (CAP antibiotics). Show NotFoundScreen → click Search Live → 3-stage progress → cited answer. "20 seconds, fully cited, and now in the cache." | Live app |
